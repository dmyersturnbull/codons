...
\textbf{Preliminary data:} Using the same data set of 437 S. cerevisiae genes, we investigated the correlation between codon usage bias and two simple measures of structural complexity:
\begin{enumerate}[a)]
...
\end{enumerate}
\todo{Fill in when run finishes [Douglas]}

In the case of structural complexity, we are interested in statistical associations that are potentially nonlinear. For this purpose we will use the Hilbert--Schmidt Independence Criterion (HSIC) \cite{Song2012} to identify statistical associations, both linear and nonlinear.
...
\todo{Does this go under benchmarking? [Robin]}
...
% Douglas: I moved this stuff
%\subsubsection{Data sets}
%Domain information will be obtained from the Structural Classification of Proteins (SCOP) database \citep{scop}.

\subsubsection{Analysis of accuracy}
\todo{Where does this go? ---Douglas [Robin]}
